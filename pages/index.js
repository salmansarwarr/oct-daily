...
const Home = ({ messages }) => {
    ...
    const toggleExpand = (messageId) => {
        if (expandedMessages.includes(messageId)) {
            setExpandedMessages(
                expandedMessages.filter((id) => id !== messageId)
            );
        } else {
                ...
                <table className="bg-gray-800 w-full mt-3 rounded-sm">
                    <thead className="text-sm md:text-base text-white w-full border-b-white border-b-2">
                        <tr>
                            <th>id</th>
                            <th>Message</th>
                            <th className="pr-4">Edit</th>
                            <th className="pr-4">Remove</th>
                        </tr>
                    </thead>
                    <tbody className="text-[0.8rem] text-gray-200  md:text-[0.94rem] w-full text-left">
                        {displayedMessages.map((message) => (
                            <tr key={message.id}>
                                <td className="w-8 text-center">{message.id}</td>
                                <td className=" py-2 px-2">
                                    ...
                                    !expandedMessages.includes(message.id) ? (
                                        <>
                                            ...
                                            <button
                                                className="text-[#E50194] font-bold  italic"
                                                onClick={() =>
                                        ...
                                        <>
                                            ...
                                            {expandedMessages.includes(
                                                ...
                                                <button
                                                    className="text-[#E50194]  italic font-bold"
                                                    onClick={() =>
                                                        toggleExpand(message.id)
                                                    }
                                                >
